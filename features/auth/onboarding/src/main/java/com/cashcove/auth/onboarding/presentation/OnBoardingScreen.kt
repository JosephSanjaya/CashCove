...
fun OnBoardingScreen(
    ...
    CashCoveContentLoader(state.onboardingDataUiState) {
        OnBoardingContent(
            onboardingList = it.onboardingPages,
            onFinishClicked = { viewModel.onIntent(OnBoardingIntent.OnboardingFinished) }
        )
    }
}
...
private fun OnBoardingContent(
    onboardingList: PersistentList<OnboardingItem>,
...
@Composable
private fun Onboarding(onboardingPages: PersistentList<OnboardingItem>) {
    val pagerState = rememberPagerState(
        initialPage = 0,
        pageCount = { onboardingPages.size })
    HorizontalPager(state = pagerState, userScrollEnabled = true) { index ->
        AsyncImage(
            model = onboardingPages[index].image,
